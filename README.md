...
В блокнотах используется **OpenAPI-совместимый** интерфейс к LLM. В начале каждого блокнота подставьте свои данные в переменные:

- **`api_key`** — ваш API-ключ (строка).
- **`base_url`** — URL провайдера, если нужен (иначе `None`; например, `"https://api.openai.com/v1/"`).

Можно подставить ключ прямо в ячейку (`api_key = "sk-..."`) или загрузить из файла **`api_key`** / **`openai_api_key`** в рабочую директорию — тогда блокнот подхватит его сам.
...
## Правки перевода и формулировок (ноутбуки)

По всем темам проведено выборочное редактирование текстов для проходящих курс: исправлены неудачный перевод и неочевидные формулировки.

**Типичные правки:**
- **«Магистр права» / «студенты LLM»** → **«модель» / «LLM» / «модели»** (путаница LLM с «магистратурой»).
- **«Приглашение»** в смысле prompt → **«промпт»** (system prompt, шаблон промпта, длина промпта).
- **«Тетрадь»** → **«блокнот»** (notebook).
- **RAG:** «Поисковая дополненная генерация» / «средство правовой защиты» → **«RAG» / «генерация с дополнением поиском (RAG)»**.
- **Заголовки:** «##Задание» → «## Задание», «Готовимся» / «Подготовка вещей» → «Подготовка» / «Подготовка среды».
- **Термины:** CoT, MapReduce, джейлбрейк, guardrails, scratchpad, KV-кэш — приведены к единому и понятному виду где нужно.

**По темам:** правки внесены в topic1 (1.1–1.7), topic2 (2.1–2.7), topic3 (3.1–3.4), topic4 (4.1–4.3), topic5 (5.2), topic6 (6.1) и в соответствующие `*_solutions.ipynb`.